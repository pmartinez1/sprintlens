...
export default function SprintProgressChart({ items }) {
  ...
  items.forEach(item => {
    ...
    if (!phaseMap[phase]) phaseMap[phase] = { phase, total: 0, done: 0, inProgress: 0, notStarted: 0 };
    phaseMap[phase].total++;
    const s = (item.status || '').toLowerCase();
    if (s.includes('done') || s.includes('complete')) phaseMap[phase].done++;
    else if (s.includes('progress') || s.includes('working')) phaseMap[phase].inProgress++;
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
      <h3 className="text-sm font-semibold text-slate-300 mb-4">Phase Progress</h3>
